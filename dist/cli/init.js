#!/usr/bin/env node
import { createRequire as __createRequire } from "node:module";import { fileURLToPath as __fileURLToPath } from "node:url";import { dirname as __dirname_fn } from "node:path";const require = __createRequire(import.meta.url);const __filename = __fileURLToPath(import.meta.url);const __dirname = __dirname_fn(__filename);
...
function writeYaml(filePath, data) {
  fs.writeFileSync(filePath, import_yaml.default.stringify(data, { lineWidth: 0 }));
}
function ensureGitignore(projectPath) {
  const gitignore = path.join(projectPath, ".gitignore");
  const lines = fs.existsSync(gitignore) ? fs.readFileSync(gitignore, "utf8").split("\n") : [];
  const needed = [];
...
function parseArgs(argv) {
  ...
  const targetVal = targetIdx >= 0 ? args[targetIdx + 1] : void 0;
  const registerTarget = targetVal && ["claude", "opencode", "both"].includes(targetVal) ? targetVal : "both";
...
async function fillSecretsInteractively(project, secrets) {
  ...
  console.log("\n=== SSH auth ===");
  const useKey = secrets[alias].ssh_key_path ? await confirm("Use SSH key (already detected)?", true) : await confirm("Use SSH key instead of password?", false);
  if (useKey) {
  ...
  if (project.db?.[alias]) {
    ...
    if (db.type !== "sqlite") {
      ...
      const useSamePwd = samePwd ? await confirm("Use SSH password for DB?", true) : false;
      if (!useSamePwd) {
    ...
    if (await confirm("\nAdd a database?", false)) {
      ...
      if (dbType === "sqlite") {
        ...
        const port2 = Number(await ask("DB port", { default: String(defaultPort(dbType)) }));
        const database = await ask("DB name", { required: true });
        const user = await ask("DB user", { default: srv.user });
        project.db = { [alias]: { type: dbType, host, port: port2, database, user } };
