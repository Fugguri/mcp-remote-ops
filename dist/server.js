...
try {
    ...
    process.stderr.write(`mcp-remote-ops: failed to load config from ${PROJECT_PATH}\n` +
        `  ${e instanceof Error ? e.message : String(e)}\n` +
        `Run: npx github:Fugguri/mcp-remote-ops init ${PROJECT_PATH}\n`);
    process.exit(1);
}
const aliasesWithoutAuth = Object.keys(config.servers).filter((alias) => !config.hasSecret(alias, "password") && !config.hasSecret(alias, "ssh_key_path"));
if (aliasesWithoutAuth.length > 0) {
    process.stderr.write(`mcp-remote-ops: warning — no SSH credentials for servers: ${aliasesWithoutAuth.join(", ")}.\n` +
...
const CONFIRM_NOTE = " If the operation requires confirmation, this tool returns " +
...
const server = new Server({ name: "mcp-remote-ops", version: "0.1.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "list_servers",
            description: "List configured server aliases and their hosts. Use this first if you don't know which alias to pass to other tools.",
        ...
        {
            name: "db_query",
            description: "Execute a SQL query. SELECT runs immediately. UPDATE/DELETE/INSERT need confirm. DDL is blocked." +
                CONFIRM_NOTE,
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "confirm_action",
            description: "Execute a previously-returned pending action by its action_id. Call only after user approval.",
            inputSchema: {
                type: "object",
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        if (name === "list_servers") {
            result = Object.fromEntries(Object.entries(config.servers).map(([alias, s]) => [
                alias,
            ...
            result = await docker.logs(String(args.server), String(args.container), Number(args.lines ?? 100));
        }
            ...
            result = await sync.sync(String(args.server), String(args.local_path), String(args.remote_path));
        }
        ...
        logger.log(name, serverArg ?? "—", String(args.command ?? args.sql ?? ""), "error", result.kind);
    }
    return {
